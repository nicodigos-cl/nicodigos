...
export const templatePreviewValues: Record<string, string> = {
  ...
  "order.total": "$19.990",
  "delivery.url": "https://nicodigos.com/dashboard/deliveries/demo",
  "product.name": "Producto digital",
  "support.email": "soporte@nicodigos.com",
};
...
export function renderCommunicationTemplate(content: string, values: Record<string, string>): string {
  const found = findTemplateVariables(content);
  for (const variable of found) {
    if (!communicationTemplateVariableSchema.safeParse(variable).success || !(variable in values)) {
      throw new Error(`UNKNOWN_TEMPLATE_VARIABLE:${variable}`);
    }
  }
  return content.replace(variablePattern, (_, variable: string) => sanitizeHtml(values[variable] ?? "", { allowedTags: [], allowedAttributes: {} }));
}
