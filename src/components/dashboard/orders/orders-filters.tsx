...
import {
  customerOrderPaymentFilterValues,
  customerOrderSortValues,
  customerOrderStatusFilterValues,
...
const statusLabels: Record<
  (typeof customerOrderStatusFilterValues)[number],
  string
> = {
  all: "Todos",
  pending: "Pendientes",
...
const paymentLabels: Record<
  (typeof customerOrderPaymentFilterValues)[number],
  string
> = {
  pending: "Pendiente",
  paid: "Pagado",
...
const deliveryLabels: Record<(typeof deliveryMethodValues)[number], string> = {
  SMM: "SMM",
  KINGUIN: "Key digital",
  MANUAL: "Entrega digital",
};

const sortLabels: Record<(typeof customerOrderSortValues)[number], string> = {
...
function countActiveFilters(query: CustomerOrdersListQuery): number {
  let count = 0;
  if (query.status && query.status !== "all") count += 1;
  if (query.payment) count += 1;
  if (query.delivery) count += 1;
...
export function OrdersFilters({ query }: { query: CustomerOrdersListQuery }) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      >
        ...
        {query.payment ? (
          <input type="hidden" name="payment" value={query.payment} />
        ) : null}
        {query.delivery ? (
          <input type="hidden" name="delivery" value={query.delivery} />
        ) : null}
        {query.sort !== "newest" ? (
            ...
            <PopoverHeader className="flex-row items-start justify-between gap-3 border-b border-border px-4 py-3">
              <div className="space-y-1">
                ...
                <PopoverDescription className="text-xs">
                  Estado, pago, entrega, fechas y orden.
                </PopoverDescription>
              </div>
              {activeFilterCount > 0 ? (
                <Button
              ...
              <div className="grid gap-0 sm:grid-cols-2 lg:grid-cols-3">
                <section className="space-y-3 border-b border-border p-4 sm:border-r lg:border-b-0">
                  ...
                  <label className="grid gap-1.5 text-xs text-muted-foreground">
                    ...
                    >
                      ...
                      {customerOrderPaymentFilterValues.map((payment) => (
                ...
                <section className="space-y-3 p-4 sm:col-span-2 lg:col-span-1">
                  ...
                  <label className="grid gap-1.5 text-xs text-muted-foreground">
                    ...
                    >
                      {customerOrderSortValues.map((sort) => (
